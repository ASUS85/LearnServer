...
.teacher-day-title h3 {
    font-size: 16px;
    margin: 0;
}
.teacher-day-title span {
    font-size: 12px;
    color: #94a3b8;
}
.teacher-event {
...
<head>
    ...
    <title>Calendrier Enseignant - LearnServer</title>
    ...
    <main class="main-content">
        <div class="page-header">
            <div class="header-info">
                <span class="badge-accent">Calendrier enseignant</span>
                <h1>Emploi du temps</h1>
                <p><?php echo htmlspecialchars($teacherName); ?>, consultez votre semaine sous forme de calendrier.</p>
            </div>
            <div class="header-actions">
                <a href="<?php echo baseUrl('teacher/dashboard'); ?>" class="btn-primary" style="text-decoration:none;display:inline-flex;align-items:center;gap:8px;">
        ...
        <section class="stats-grid">
            <div class="stat-card glass-panel">
                ...
                <div class="stat-details">
                    <h3>Séances</h3>
                    <p class="stat-number"><?php echo count($weeklySchedule ?? []); ?></p>
                </div>
            </div>
            <div class="stat-card glass-panel">
                <div class="stat-icon purple"><i data-lucide="clock-3"></i></div>
                <div class="stat-details">
                    <h3>Aujourd'hui</h3>
                    <p class="stat-number"><?php echo htmlspecialchars($today ?? ''); ?></p>
                </div>
            </div>
        </section>
        ...
        <section class="glass-panel" style="padding:24px;">
            <div class="panel-header" style="margin-bottom: 0;">
                ...
                <span class="live-badge">Semaine complète</span>
            </div>
                        ...
                        <?php if (!empty($sessions)): ?>
                            <?php foreach ($sessions as $session): ?>
                                <div class="teacher-event">
                                    <div class="teacher-event-time">
                                        <?php echo htmlspecialchars(substr($session['heure_debut'] ?? '', 0, 5)); ?> - <?php echo htmlspecialchars(substr($session['heure_fin'] ?? '', 0, 5)); ?>
                                    </div>
                                    <h4><?php echo htmlspecialchars($session['nom_matiere'] ?? 'Matière'); ?></h4>
                                    <p>
                                        <?php echo htmlspecialchars(($session['nom_filiere'] ?? '') . ' • ' . ($session['nom_niveau'] ?? '')); ?><br>
                                        Salle / classe: <?php echo htmlspecialchars($session['salle'] ?? $session['nom_classe'] ?? 'Non renseigné'); ?>
                                    </p>
                                </div>
                            <?php endforeach; ?>
                        <?php else: ?>
